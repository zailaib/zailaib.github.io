...
export function buildInterior(houseGroup, parts, MATS) {
  ...
  // ═══════════════════════════════════════════════════════════════
  // GRAIN STORAGE (upstairs, center bay area)
  // Grain barrels and sacks
  // ═══════════════════════════════════════════════════════════════
  const grainGrp = partGrp('grainStore', '粮仓(二层)', '#8b6914',
    ['upperWallFront','upperWallBack','upperWallLeft','upperWallRight']);

  const grainY = BAND_Y + 0.15; // on upper floor level

  // Grain barrels (cylindrical)
  for (let i = 0; i < 3; i++) {
    const barrelGeo = new THREE.CylinderGeometry(0.35, 0.32, 0.8, 10);
    const barrel = new THREE.Mesh(barrelGeo, MATS.grainMat);
    barrel.position.set(-1.5 + i * 1.5, grainY + 0.4, -HD2 + 1.0);
    addTo('grainStore', barrel); grainGrp.add(barrel);
    // Lid
    const lid = box(0.72, 0.05, 0.72, MATS.woodLight);
    lid.position.set(-1.5 + i * 1.5, grainY + 0.82, -HD2 + 1.0);
    addTo('grainStore', lid); grainGrp.add(lid);
  }

  // Grain sacks (squat rounded shapes)
  for (let i = 0; i < 4; i++) {
    const sackGeo = new THREE.CylinderGeometry(0.25, 0.3, 0.5, 8);
    const sack = new THREE.Mesh(sackGeo, MATS.grainSack);
    sack.position.set(-1.8 + i * 1.2, grainY + 0.25, HD2 - 1.0);
    addTo('grainStore', sack); grainGrp.add(sack);
    // Rope tie at top
    const tieGeo = new THREE.TorusGeometry(0.22, 0.03, 6, 8);
    const tie = new THREE.Mesh(tieGeo, MATS.woodDark);
    tie.position.set(-1.8 + i * 1.2, grainY + 0.5, HD2 - 1.0);
    tie.rotation.x = Math.PI/2;
    addTo('grainStore', tie); grainGrp.add(tie);
  }

  // Farming tools hanging on upper back wall
  for (let i = 0; i < 3; i++) {
    const toolHandle = cyl(0.02, 0.8, MATS.woodDark, 6);
    toolHandle.position.set(-2 + i * 2, grainY + 0.8, -HD2 + WALL_T/2 + 0.1);
    toolHandle.rotation.x = Math.PI/2;
    addTo('grainStore', toolHandle); grainGrp.add(toolHandle);
    // Tool head
    const toolHead = box(0.25, 0.04, 0.12, MATS.wokMetal);
    toolHead.position.set(-2 + i * 2, grainY + 0.4, -HD2 + WALL_T/2 + 0.15);
    addTo('grainStore', toolHead); grainGrp.add(toolHead);
  }

  // Baskets / woven containers
  for (let i = 0; i < 2; i++) {
    const basketGeo = new THREE.CylinderGeometry(0.2, 0.15, 0.3, 8);
    const basket = new THREE.Mesh(basketGeo, MATS.woodLight);
    basket.position.set(2 + i * 0.5, grainY + 0.15, 0);
    addTo('grainStore', basket); grainGrp.add(basket);
  }
}
