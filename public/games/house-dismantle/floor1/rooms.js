...
export function buildFloor1Rooms(houseGroup, parts, MATS) {
  ...
  // Bed against left wall (x=-7.5, z=2), wardrobe near door (x=-5, z=0.5)
  const er1 = parts.get('elderRoom1').group;
  er1.add(makeBed(-7, 2.2, F, 'left'));
  const w1 = box(1.2, 2.0, 0.6, MATS.woodDark); w1.position.set(-5, 1.0 + F, 1.0); w1.userData.partName = 'elderRoom1'; w1.castShadow = true;
  er1.add(w1); parts.get('elderRoom1').meshArr.push(w1);

  // ── Elder room 2 (Bay2 front: x=-4..0, z=0..4.5) ──
  // Bed against back wall (x=-2, z=-3.5), wardrobe near doorway (x=-2, z=0.5)
  const er2 = parts.get('elderRoom2').group;
  er2.add(makeBed(-2, -3.5, F, 'back'));
  const w2 = box(1.2, 2.0, 0.6, MATS.woodDark); w2.position.set(-1.5, 1.0 + F, 1.0); w2.userData.partName = 'elderRoom2'; w2.castShadow = true;
  er2.add(w2); parts.get('elderRoom2').meshArr.push(w2);

  // ── Living room (Bay3 front: x=0..4, z=0..4.5) ──
  const lr = parts.get('livingRoom').group;
  const lt = box(2.2, 0.06, 1.6, MATS.woodLight); lt.position.set(2, 0.85 + F, 2.8); addTo('livingRoom', lt); lr.add(lt);
  for (const [lx, lz] of [[-0.95, -0.65],[0.95, -0.65],[-0.95, 0.65],[0.95, 0.65]]) {
    const leg = box(0.08, 0.8, 0.08, MATS.woodDark); leg.position.set(2 + lx, 0.4 + F, lz + 2.8); addTo('livingRoom', leg); lr.add(leg);
  }
  for (const [sx, sz] of [[2, 1.5],[2, 4.1],[0.7, 2.8],[3.3, 2.8]]) lr.add(makeChair(sx, sz, F));
  // Tea cabinet against front wall (z=4.0), away from doorway
  const tc = box(1.4, 1.5, 0.4, MATS.woodDark); tc.position.set(2, 0.75 + F, 4.0); addTo('livingRoom', tc); lr.add(tc);
